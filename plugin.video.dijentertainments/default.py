...
CIBase = 'aHR0cDovL2Rpai56b21iaWUtd2l6YXJkLnVrL2xpc3RzL3htbC9ob21lLnhtbA=='.decode('base64')

addon = xbmcaddon.Addon('plugin.video.dijentertainments')
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(dijentertainments,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(dijentertainments,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(dijentertainments,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('dijentertainments Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('dijentertainments', 'Nothing Entered')
          return	   
    ...
    progress.create('dijentertainments Searching Please wait',' ')
