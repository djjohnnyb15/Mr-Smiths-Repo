    ...
    if not encode: return False
        ...
        if type=='Movies':
            if selfAddon.getSetting('ssm_iwatch') != 'false':
                ...
                results.append(q)
            if selfAddon.getSetting('ssm_Architects@Work') != 'false':
                sources.append('Architects@Work')
                q = queue.Queue()
                threading.Thread(target=Architects@Work,args=(encode,type,q)).start()
                results.append(q)
...
def Architects@Work(encode,type,q):
    from resources.libs import Architects@Work
    returnList = Architects@Work.superSearch(encode,type)
