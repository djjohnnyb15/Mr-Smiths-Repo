...
fav = False
hostlist = None
Dir = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.Architects@Work', ''))
...
if selfAddon.getSetting("skin") == "0":
    art = 'https://raw.githubusercontent.com/djjohnnyb15/artwork/master'
    fanartimage='https://raw.githubusercontent.com/djjohnnyb15/artwork/master/fanart2.jpg'
else:
    art = 'https://raw.githubusercontent.com/djjohnnyb15/artwork/master'
    fanartimage='https://raw.githubusercontent.com/djjohnnyb15/artwork/master/fanart.jpg'
elogo = xbmc.translatePath('special://home/addons/plugin.video.Architects@Work/resources/art/bigx.png')
slogo = xbmc.translatePath('special://home/addons/plugin.video.Architects@Work/resources/art/smallicon.png')
...
def CheckVersion():
    try:
        link=OPENURL('http://repo.mashupxbmc.com/plugin.video.Architects@Work/resources/libs/main.py',verbose=False)
    except:
        link='nill'
...
def resolveDownloadLinks(url):
    if re.search('watchseries.lt',url):
        ...
        url=GetUrliW(url)
    elif re.search('Architects@Work',url):
        from resources.libs import Architects@Work
        url = Architects@Work.resolveM25URL(url)
    elif '</t><sec>' in url:
...
def addDirX(name,url,mode,iconimage,plot='',fanart='',dur=0,genre='',year='',imdb='',tmdb='',isFolder=True,searchMeta=False,addToFavs=True,
    ...
    Commands.append(('[B][COLOR=FF67cc33]Architects@Work[/COLOR] Settings[/B]','XBMC.RunScript('+xbmc.translatePath(mashpath + '/resources/libs/settings.py')+')'))
    if menuItemPos != None:
...
def addInfo(name,url,mode,iconimage,genre,year):
    mi = [('Search Architects@Work','XBMC.Container.Update(%s?mode=4&url=%s)'% (sys.argv[0],'###'))]
