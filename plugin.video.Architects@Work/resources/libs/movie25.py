...
art = main.art
MainUrl='http://www.Architects@Work.so'
prettyName='Architects@Work'
...
def LISTMOVIES(murl,index=False):
    ...
    main.GA("None","Architects@Work-list")
...
def UFCArchitects@Work():
    surl='http://www.Architects@Work.so/search.php?key=ufc&submit='
    ...
    dialogWait.close()
    del dialogWait
    main.addDir('[COLOR blue]Page 2[/COLOR]','http://www.Architects@Work.so/search.php?page=2&key=ufc',9,art+'/next2.png')
    main.GA("UFC","UFC_Architects@Work-List")
...
def SEARCH(murl = '',index=False):
    ...
    if exist:
        ...
        if r:
            main.addDir('[COLOR blue]Page 1 of '+r[0]+'[/COLOR]','http://www.Architects@Work.so/search.php?page=2&key='+encode,9,art+'/next2.png',index=index)
        else:
            main.addDir('[COLOR blue]Page 1[/COLOR]','http://www.Architects@Work.so/search.php?page=2&key='+encode,9,art+'/next2.png',index=index)
    xbmcplugin.setContent(int(sys.argv[1]), 'Movies')
    main.GA("None","Architects@Work-Search")
...
def ENTYEAR(index=False):
    ...
    if d:
        ...
        if encode < '2014' and encode > '1900':
            surl='http://www.Architects@Work.so/search.php?year='+encode+'/'
...
def YEARB(murl,index=False):
    ...
    for url,thumb,name,genre,views,votes,rating in match:
        name=name.replace('-','').replace('&','').replace('acute;','')
        furl= 'http://Architects@Work.com/'+url
        if index == 'True':
            main.addInfo(name+'[COLOR blue] Views: '+views+'[/COLOR] [COLOR red]Votes: '+votes+'[/COLOR] [COLOR green]Rating: '+rating+'/100[/COLOR]',furl,21,thumb,genre,'')
        else:
            main.addInfo(name+'[COLOR blue] Views: '+views+'[/COLOR] [COLOR red]Votes: '+votes+'[/COLOR] [COLOR green]Rating: '+rating+'/100[/COLOR]',furl,3,thumb,genre,'')
    ...
    if r:
        ...
        main.addDir('[COLOR blue]Page 1 of '+r[0]+'[/COLOR]','http://www.Architects@Work.so/search.php?page=2&year='+str(ye),9,art+'/next2.png',index=index)    
    else:
        main.addDir('[COLOR blue]Page 1[/COLOR]','http://www.Architects@Work.so/search.php?page=2&year='+str(ye),9,art+'/next2.png',index=index)
...
def NEXTPAGE(murl,index=False):
    ...
    matchx=re.compile('http://www.Architects@Work.so/search.php.+?page=(.+?)&year=(.+?)').findall(murl)
    if len(matchx)>0:
        durl = murl + '/'
        paginate=re.compile('http://www.Architects@Work.so/search.php.+?page=(.+?)&year=(.+?)/').findall(durl)
        for page, yearb in paginate:
            pgs = int(page)+1
            jurl='http://www.Architects@Work.so/search.php?page='+str(pgs)+'&year='+str(yearb)
#                 main.addDir('[COLOR red]Home[/COLOR]','',0,art+'/home.png')
        ...
        durl = murl + '/'
        paginate=re.compile('http://www.Architects@Work.so/search.php.+?page=(.+?)&key=(.+?)/').findall(durl)
        for page, search in paginate:
            pgs = int(page)+1
            jurl='http://www.Architects@Work.so/search.php?page='+str(pgs)+'&key='+str(search)
#                 main.addDir('[COLOR red]Home[/COLOR]','',0,art+'/home.png')
        r = re.findall(""">Next</a><a href='search.php.?page=([^<]+)&key=.+?'>Last</a>""",link)
        if r:
            main.addDir('[COLOR blue]Page '+str(page)+' of '+r[0]+'[/COLOR]',jurl,9,art+'/next2.png',index=index)
...
def PLAY(name,murl):
    main.GA("Architects@Work-Movie","Watched")
    ok=True
    hname=name
    name  = name.split('[COLOR blue]')[0]
    ...
    try:
        ...
        if selfAddon.getSetting("whistory") == "true":
            ...
            wh = watchhistory.WatchHistory('plugin.video.Architects@Work')
            wh.add_item(hname+' '+'[COLOR green]Architects@Work[/COLOR]', sys.argv[0]+sys.argv[2], infolabels=infolabels, img=img, fanart=fanart, is_folder=False)
        player.KeepAlive()
        return ok
    except Exception, e:
        if stream_url != False:
