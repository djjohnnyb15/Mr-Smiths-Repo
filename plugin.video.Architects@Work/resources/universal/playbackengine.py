...
def QueueItem(addon_id, title, url, is_resolved=False,img='', fanart='', infolabels=''):
    ...
    pbe_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (script_path, sys.argv[1], '?' + common.dict_to_paramstr(pbe_params), 'plugin.video.Architects@Work.playbackengine')
...
if sys.argv and len(sys.argv) >= 4 and sys.argv[3] == 'plugin.video.Architects@Work.playbackengine':        
    
    sys.argv[0] = 'plugin.video.Architects@Work'
    addon_pbe= Addon('plugin.video.Architects@Work', sys.argv)
