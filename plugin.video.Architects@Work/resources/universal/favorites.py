...
class Favorites:
    ...
    def add_item(self, title, url, fmtd_title='', section_title='Misc.', section_addon_title='', sub_section_title='', sub_section_addon_title='', infolabels='', img='', fanart='', is_playable=False, is_folder=False):
        ...
        uni_fav_add_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (self._get_script_path(), self.sys_argv[1], self._build_params(uni_fav, infolabels), 'plugin.video.Architects@Work.favorites')
    ...
    def delete_item(self, title, fmtd_title='', item_mode='main', section_title='Misc.', section_addon_title='', sub_section_title='', sub_section_addon_title=''):
        ...
        uni_fav_add_script = 'XBMC.RunScript(%s, %s, %s, "%s")' % (self._get_script_path(), self.sys_argv[1], self._build_params(uni_fav), 'plugin.video.Architects@Work.favorites')
        
        return uni_fav_add_script
    
    def _get_script_path(self):
        return os.path.join('special://home/addons/plugin.video.Architects@Work','resources','universal','favorites.py')
...
if sys.argv and len(sys.argv) >= 4 and sys.argv[3] == 'plugin.video.Architects@Work.favorites':    
    
    sys.argv[0] = 'plugin.video.Architects@Work'
    addon_fav = Addon('plugin.video.Architects@Work', sys.argv)
